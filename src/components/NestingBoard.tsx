...
export function NestingBoard({
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    const currentItem = items.find((i) => i.id === draggingRef.current!.id);
    ...
    items.forEach((other) => {
      if (other.id === draggingRef.current!.id) return;
      if (Math.abs(newX - other.x) < snapThreshold) newX = other.x;
      ...
      if (Math.abs(newY - other.y) < snapThreshold) newY = other.y;
    ...
    onChange(
      items.map((i) =>
        i.id === draggingRef.current!.id ? { ...i, x: newX, y: newY } : i
      )
    );
  };
